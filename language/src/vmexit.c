...
void ring_vm_exit(VM *pVM, int nType) {
	...
	if (ring_list_getsize(pActiveList) > 0) {
		...
		if ((nStep > 0) && (nStep <= ring_list_getsize(pActiveList))) {
			...
			while (ring_list_getsize(pActiveList) > x) {
				ring_list_deleteitem_gc(pVM->pRingState, pVM->pExitMark,
							ring_list_getsize(pActiveList));
				ring_list_deleteitem_gc(pVM->pRingState, pVM->pLoopMark,
							ring_list_getsize(pActiveList));
			}
		} else {
			if (nType == RING_COMMANDTYPE_EXIT) {
...
void ring_vm_stepfromreg(VM *pVM) { ring_list_adddouble_gc(pVM->pRingState, pVM->pForStep, RING_VM_IR_READD); }
