...
RING_API List *ring_list_delete_gc(void *pState, List *pList) { return ring_list_deleteref_gc(pState, pList); }
	...
	*/
	...
	for (x = 1; x <= nMax; x++) {
		if (ring_list_isint(pList, x)) {
			...
			ring_list_addstring2_gc(pState, pNewList, ring_list_getstring(pList, x),
...
RING_API Item *ring_list_getitem_gc(void *pState, List *pList, unsigned int nIndex) {
	...
	if (nIndex > 0 && nIndex <= ring_list_getsize(pList)) {
		...
		if (lUseListCache) {
			...
			else if ((nIndex > pList->nNextItem) &&
				 ((nIndex - pList->nNextItem) < (pList->nSize - nIndex)) &&
				 (pList->pLastItem != NULL)) {
			...
			else if ((nIndex < pList->nNextItem) && ((pList->nNextItem - nIndex) < nIndex) &&
				 (pList->pLastItem != NULL)) {
				pItems = pList->pLastItem;
				for (x = pList->nNextItem - 1; x >= nIndex; x--) {
					if (x == nIndex) {
...
RING_API void ring_list_insertitem_gc(void *pState, List *pList, unsigned int x) {
	...
	*/
	ring_list_getitem_gc(pState, pList, x);
...
RING_API List *ring_list_insertlist_gc(void *pState, List *pList, unsigned int nPos) {
	...
	List *pList2;
	ring_list_insertitem_gc(pState, pList, nPos);
	pItem = ring_list_getitem_gc(pState, pList, nPos + 1);
	ring_item_settype_gc(pState, pItem, ITEMTYPE_LIST);
	pList2 = ring_item_getlist(pItem);
	return pList2;
}
...
RING_API unsigned int ring_list_binarysearchstr_gc(void *pState, List *pList, const char *cFind, unsigned int nColumn,
	...
	while (nFirst <= nLast) {
		nMid = (nFirst + nLast) / 2;
		nRes = strcmp(ring_list_getstringcolumn_gc(pState, pList, nMid, nColumn, cAttribute), cFind);
		if (nRes == 0) {
			return nMid;
		} else if (nRes < 0) {
			nFirst = nMid + 1;
...
RING_API void ring_list_genhashtable2_gc(void *pState, List *pList) {
	...
	for (x = 1; x <= ring_list_getsize(pList); x++) {
		if (ring_list_islist(pList, x)) {
			pList2 = ring_list_getlist(pList, x);
			if ((ring_list_getsize(pList2) >= RING_LIST_KEYINDEX) &&
			    (ring_list_isstring(pList2, RING_LIST_KEYINDEX))) {
				ring_hashtable_newpointer_gc(pState, pList->pHashTable,
							     ring_list_getstring(pList2, RING_LIST_KEYINDEX), pList2);
			}
		}
	}
}
...
RING_API void ring_list_genarray(List *pList) { ring_list_genarray_gc(NULL, pList); }

RING_API List *ring_list_delete(List *pList) { return ring_list_delete_gc(NULL, pList); }

RING_API void ring_list_deletearray(List *pList) { ring_list_deletearray_gc(NULL, pList); }
...
RING_API void ring_list_addint(List *pList, int x) { ring_list_addint_gc(NULL, pList, x); }
/* Pointers */
...
RING_API void ring_list_addpointer(List *pList, void *pValue) { ring_list_addpointer_gc(NULL, pList, pValue); }
/* Function Pointers */
...
RING_API void ring_list_adddouble(List *pList, double x) { ring_list_adddouble_gc(NULL, pList, x); }
/* String */
...
RING_API void ring_list_addstring(List *pList, const char *cStr) { ring_list_addstring_gc(NULL, pList, cStr); }
...
RING_API List *ring_list_newlist(List *pList) { return ring_list_newlist_gc(NULL, pList); }

RING_API void ring_list_setlist(List *pList, unsigned int nIndex) { ring_list_setlist_gc(NULL, pList, nIndex); }

RING_API void ring_list_copy(List *pNewList, List *pList) { ring_list_copy_gc(NULL, pNewList, pList); }

RING_API void ring_list_deleteallitems(List *pList) { ring_list_deleteallitems_gc(NULL, pList); }
...
RING_API void ring_list_insertitem(List *pList, unsigned int x) { ring_list_insertitem_gc(NULL, pList, x); }
	...
	for (x = 1; x <= ring_list_getsize(pList); x++) {
		if (ring_list_isstring(pList, x)) {
			...
			if (ring_list_isobject(pList2)) {
				...
				if (ring_list_isref_gc(pState, pList2)) {
...
RING_API void ring_list_addcpointer(List *pList, void *pGeneral, const char *cType) {
	ring_list_addcpointer_gc(NULL, pList, pGeneral, cType);
}
